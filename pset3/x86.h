 ...
 *   - struct registers: Used in process descriptors to store x86 registers.
...
#define PTE_P           ((pageentry_t) 1) // Page table entry is Present
#define PTE_W           ((pageentry_t) 2) // Page table entry is Writeable
#define PTE_U           ((pageentry_t) 4) // Page table entry is User-accessible
...
/*****************************************************************************
   struct registers: A complete set of x86 general-purpose registers, plus
   some special-purpose registers. The order and contents are defined to make
...
static inline void lcr3(pagedirectory_t val) {
    asm volatile("movl %0,%%cr3" : : "r" (val));
}

static inline pagedirectory_t rcr3(void) {
    pagedirectory_t val;
